...
public class NewsInformationVM extends BaseViewModel {
     ...
     * 新闻列表
     */
    public MutableLiveData<TJBean> getNewsList(String type, int page) {
        final MutableLiveData<TJBean> liveData = new MutableLiveData<>();
        RetrofitManager.newInstance().creat(APIService.class).getNewsList(type, page, 30, 1).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Observer<TJBean>() {
            @Override
            ...
            @Override
            public void onNext(@NonNull TJBean tjBean) {
                liveData.postValue(tjBean);
            }
